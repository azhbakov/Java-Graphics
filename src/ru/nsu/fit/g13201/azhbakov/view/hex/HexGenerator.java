...
public class HexGenerator {
    private static int h, r, s, t, edge, cornerEdge;
    private static int imageWidth;
    private static int imageHeight;

    public static void initGenerator (int hexSide, int edge) {
        HexGenerator.edge = edge;
        setSide(hexSide);
    ...
    public static Hex drawHex (Color hexColor) {
        int centerX = edge+r;
        int centerY = cornerEdge+s/2+t;
        BufferedImage hexImage = new BufferedImage(imageWidth, imageHeight, BufferedImage.TYPE_INT_ARGB);
        //fillCanvas (hexImage, Color.red);

        // Init Bresenham movement along up right edge
        XBresenhemIterator brInner = new XBresenhemIterator(HexGenerator.getHexVertexX(centerX, 0), HexGenerator.getHexVertexY(centerY, 0),
                HexGenerator.getHexVertexX(centerX, 1), HexGenerator.getHexVertexY(centerY, 1)); // go along inner up right edge
        int side = s;
        setHeight(h + 2*edge);
        ...
        //System.out.println(hexImage.getWidth());
        SpanFilling.fill(hexImage, centerX, centerY, hexColor);
        return new Hex(hexImage, h, r, s, t, edge, cornerEdge);
    }

    public static void fillCanvas(BufferedImage image, Color c) {
        int color = c.getRGB();
    ...
    static class XBresenhemIterator {
        int x1, y1, x2, y2;
        int x, y;
    ...
    private static void setPixelInHex (BufferedImage im, int centerX, int centerY, int dx, int dy, Color c) {
    ...
    public static int getHexVertexX (int centerX, int vertexNum) {
        switch (vertexNum) {
            case 0:
    ...
    public static int getHexVertexY (int centerY, int vertexNum) {
        switch (vertexNum) {
